...
db = MySQLdb.connect(host="localhost",port=3306,user="root",passwd="",db="nscreen_members",charset='utf8')
cursor = db.cursor()

def execute_query_nscreen(query,vital=True,level=0):
    ...
    try:
        ...
        if vital:
            time.sleep(5)
            return execute_query_nscreen(query,True,level=level+1)
        else:
            return False
