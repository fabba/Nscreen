...
//definitions of implicit preference indicators
var logFile = "";
var startDatetime = "";
var endDatetime = "";
...
//context informations; these should be updated via initTracing method

var objectsListed = new Array();
//structure is objectsListed["oid"] = "posX, posY, type"
...
function getObjectId(element) {
    ...
    while(typeof element != "null" && typeof element != "undefined" && element.tagName != "BODY" && i < maxIterations) {
        ...
        if(element.className != undefined){
            if(element.className.match(/(?:^|\s)object(?!\S)/) ){
                oid = element.id;
                return oid;
            }
        }
...
/**
 *initialize tracing of the user, set objects and window properties
 *all Implicit Preference Indicators and context information are called through this method 
 *e = event onLoad
 */
function initTracing(e){
    ...
    //populating list of objects
    //each object has class=object and id="oid"
    var divs = document.getElementsByClassName('object');
  ...
  /*
    adding listeners on user actions, each listener updates appropriate IPIs and creates an entry to the log file
   */
        //mouse clicks
    ...
    /*all links leading towards object needs to have class=object_link*/
    ...
    sendVisit();
    var interval = setInterval(function(){sendVisit()}, 30000);
}

/**
 *user starts purchase process e.g. click on add to chart button
 */
...
/**
 *send feedback to the server, this action is repeated every 30 seconds
 */
function sendVisit(){
    endDatetime = new Date();
    storeVisit( userID, objectID, pageID, sessionID, pageType, visitID);
...
function exitTracing(){
 ...
 /*
 store traced user behavior on the server-side
 */
function storeVisit(userID, objectID, pageID, sessionID, pageType, visitID) {
	var paramString = "";
...
//objectsListed["oid"] = "posX, posY, type"
	...
	// send request
	send_xmlhttprequestTrace(readyStateRoutine, 'POST', 'IPIget-master/component/server-side/StoreVisit.php',paramString, {"Content-Type" : "application/x-www-form-urlencoded"});
}
